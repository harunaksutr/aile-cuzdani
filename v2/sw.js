...
self.addEventListener('push', e => {
  ...
  e.waitUntil(
    self.registration.showNotification(data.title || 'Aile Cüzdanı', {
      ...
      badge: '/icon-192.png',
      tag: data.tag || 'taksit',
      data: { url: data.url || 'https://www.familywallet.co/' }
    })
  );
});
...
self.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(
    clients.matchAll({ type: 'window' }).then(list => {
      for (const c of list) {
        if (c.url.includes('familywallet.co') && 'focus' in c) return c.focus();
      }
      return clients.openWindow(e.notification.data?.url || 'https://www.familywallet.co/');
    })
  );
});
